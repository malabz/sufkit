...
- `sufkit::GenomeReference` to validate and normalize FASTA or in-memory
  records;
- `sufkit::SuffixArray` for exact search, direct SA access, and MEM search;
...
only A/C/G/T after case normalization. MEM query non-ACGT symbols are hard
breaks. Built or loaded indexes are immutable and support concurrent const

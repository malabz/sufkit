...
double current_process_peak_rss_mb() {
    struct rusage usage {};
    if (getrusage(RUSAGE_SELF, &usage) != 0) {
        throw Error(ErrorCode::io_error, "cannot read benchmark worker peak RSS");
    }
...
const QueryCase& canary_query_for(const Dataset& dataset) {
    ...
    if (dataset.queries.empty()) {
        throw Error(ErrorCode::invalid_input, "benchmark dataset has no queries");
    }
...
MethodResult run_build_worker(
    ...
    if (is_sa_method(method)) {
        ...
        build_options.backend = is_caps_method(method) ? SaBackend::caps : SaBackend::divsufsort;
...
MethodResult run_load_worker(
    ...
    raw.system_seconds = cpu.system;
    if (index.suffix_array &&
        index.suffix_array->sampling_rate() != sampling_rate_for_method(method)) {
        throw Error(ErrorCode::build_failure,
            "loaded suffix-array sampling rate does not match benchmark method");
    }
    verify_canary(
        build_result,
        index.locate(canary_query_for(dataset).sequence, StrandMode::forward, std::nullopt),
        "load");
    result.loads.push_back(raw);
    return result;
}
        ...
        if (indexed != results.end()) {
            ...
            for (const auto& group : dataset.groups) {
                ...
                for (std::size_t sample = 0; sample < sample_count; ++sample) {
                    ...
                    for (const auto strands : {StrandMode::forward,
                                               StrandMode::reverse_complement,
                                               StrandMode::both}) {

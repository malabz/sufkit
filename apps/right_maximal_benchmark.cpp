        ...
        if (name == "--workload") {
            ...
            if (name == "--profile") result.profile = value;
                ...
                if (parsed == 0 || parsed > std::numeric_limits<std::uint32_t>::max())
                    throw Error(ErrorCode::invalid_input, "--build-repetitions must be a positive 32-bit integer");
                ...
                if (parsed == 0 || parsed > std::numeric_limits<std::uint32_t>::max())
                    throw Error(ErrorCode::invalid_input, "--query-repetitions must be a positive 32-bit integer");
                ...
                if (parsed > std::numeric_limits<std::uint32_t>::max())
                    throw Error(ErrorCode::invalid_input, "--warmups must be a 32-bit integer");
...
std::uint64_t unordered_match_hash(std::size_t query_id, const RightMaximalMatch& match) {
        ...
        for (std::size_t query_position = 0;
            ...
            for (std::size_t reference_position = 0;
                ...
                result.push_back({
                    ...
                    query_position, length, Strand::forward});
            }
        }
    }
...
std::vector<CorrectnessResult> run_naive_oracle(
    ...
    const auto reference = GenomeReference::from_records(reference_subset);
    SuffixArrayBuildOptions build_options;
    build_options.acceleration = SaAcceleration::none;
    ...
    for (const auto min_length : options.min_lengths) {
        ...
        right_maximal_options.algorithm = RightMaximalSearchAlgorithm::baseline;
        std::uint64_t checksum = 0;
        for (std::size_t query_id = 0; query_id < query_subset.size(); ++query_id) {
            const auto expected = naive_right_maximal_matches(
                reference_subset, query_subset[query_id].sequence, min_length);
            const auto observed = index.find_right_maximal_matches(
                query_subset[query_id].sequence, right_maximal_options);
            if (observed.total_matches != expected.size() ||
                observed.matches.size() != expected.size() ||
                !std::equal(observed.matches.begin(), observed.matches.end(),
                ...
                throw Error(ErrorCode::build_failure,
                    "naive right-maximal oracle mismatch for " + dataset.name +
...
SaAcceleration acceleration_for(const std::string& method) {
    ...
    if (method == "right-maximal-suffix-link" || method == "right-maximal-suffix-link-binary" ||
        method == "right-maximal-suffix-link-sapling" ||
        method == "right-maximal-sampled-k4" || method == "right-maximal-sampled-k8")
        return SaAcceleration::lcp_suffix_link;
...
RightMaximalSearchAlgorithm algorithm_for(const std::string& method) {
    ...
    if (method == "right-maximal-suffix-link" || method == "right-maximal-suffix-link-binary" ||
        method == "right-maximal-suffix-link-sapling" ||
        method == "right-maximal-sampled-k4" || method == "right-maximal-sampled-k8")
        return RightMaximalSearchAlgorithm::suffix_link;
...
OperationMeasurement measure_operation(
    ...
    for (std::size_t query_id = 0; query_id < dataset.queries.size(); ++query_id) {
        ...
        if (operation.streaming) {
            index.for_each_right_maximal_match(
            ...
            const auto result = index.find_right_maximal_matches(
                dataset.queries[query_id].sequence, options, operation.max_matches);
...
void benchmark_internal(const Dataset& dataset, const Options& options, const std::string& method,
    ...
    for (std::uint32_t repetition = 0; repetition < options.build_repetitions; ++repetition) {
        ...
        const auto usage = run_isolated_worker(error_path, [&] {
            ...
            phase.auxiliary_bytes = index.info().auxiliary_bytes;
            phase.learned_index_bytes = index.info().learned_index_bytes;
                ...
                const auto usage = run_isolated_worker(error_path, [&] {
                    ...
                    for (std::uint32_t warmup = 0;
                        ...
                        (void)measure_operation(
                            index, dataset, right_maximal_options, operation);
                    }
...
void benchmark_mummer(const Dataset& dataset, const Options& options,
    ...
    for (std::uint32_t repetition = 0; repetition < options.build_repetitions; ++repetition) {
        ...
        if (process.status != 0)
            throw Error(ErrorCode::build_failure, "MUMmer4 index construction failed");
    ...
    for (const auto length : options.min_lengths) {
        ...
        RightMaximalOptions forward_preflight_options;
        forward_preflight_options.min_length = length;
        forward_preflight_options.algorithm =
            RightMaximalSearchAlgorithm::baseline;
        const auto forward_preflight = measure_operation(
            verification_index, dataset, forward_preflight_options,
            operation_specs().front());
        auto reverse_preflight_options = forward_preflight_options;
        reverse_preflight_options.strands = StrandMode::reverse_complement;
        const auto reverse_preflight = measure_operation(
            verification_index, dataset, reverse_preflight_options,
            operation_specs().front());
        ...
        for (std::uint32_t warmup = 0; warmup < options.warmups; ++warmup) {
            ...
            if (run_process(args, output, error).status != 0)
                throw Error(ErrorCode::build_failure, "MUMmer4 query warm-up failed");
        }
        for (std::uint32_t repetition = 0;
             repetition < options.query_repetitions; ++repetition) {

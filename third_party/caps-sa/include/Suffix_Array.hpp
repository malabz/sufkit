...
private:
    ...
    static T_* allocate(idx_t size)
    {
        if (size == 0)
            return nullptr;
        if (static_cast<std::uint64_t>(size) >
            static_cast<std::uint64_t>(std::numeric_limits<std::size_t>::max() / sizeof(T_)))
            throw std::bad_array_new_length();
        auto* const result = static_cast<T_*>(std::malloc(static_cast<std::size_t>(size) * sizeof(T_)));
        if (result == nullptr)
            throw std::bad_alloc();
        return result;
    }

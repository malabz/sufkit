...
#if defined(CAPS_SA_ENABLE_DIAGNOSTICS)
#define CAPS_SA_LOG(statement) do { statement; } while (false)
#else
#define CAPS_SA_LOG(statement) do { } while (false)
#endif
    ...
    part_ruler_(allocate<idx_t>(p_ * (p_ + 1)))
{
    ...
    if(p_ > n_)
    {
        throw std::invalid_argument("CaPS-SA subproblem count exceeds text length");
    }
}

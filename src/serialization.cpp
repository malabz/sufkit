...
std::string ReadString(std::istream& input, const char* field) {
...
IndexInfo IndexInfoFromContainer(const ParsedContainer& container) {
  ...
  if (container.spec.kind == IndexKind::kSuffixArray) {
    ...
    for (const auto& section : container.sections) {
      if (section.type == SectionType::kInverseSuffixArray ||
          section.type == SectionType::kLcp ||
          section.type == SectionType::kChild) {
    ...
    if (learned) {
      ...
      info.learned_index_bytes =
          RequireSection(container, SectionType::kLearnedSa).size;
    }
    if (sampled) {

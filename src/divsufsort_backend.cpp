...
DivSufsortBuildResult<Index> build_divsufsort(
    ...
    if constexpr (std::is_same_v<Index, std::int32_t>) {
        if (text.size() > static_cast<std::uint64_t>(std::numeric_limits<saidx_t>::max()))
        ...
        if (text.size() > static_cast<std::uint64_t>(std::numeric_limits<saidx64_t>::max()))
    ...
    if (status != 0) throw Error(ErrorCode::build_failure, "libdivsufsort construction failed");
    ...
    result.sa_seconds = std::chrono::duration<double>(
        std::chrono::steady_clock::now() - sa_begin).count();
    if (!retain_lcp && !retain_isa) return result;
    ...
    if (thread_count <= 1 || count < (1U << 20)) {
        for (std::uint64_t row = 0; row < count; ++row) {
            const auto suffix = static_cast<std::uint64_t>(result.suffix_array[static_cast<std::size_t>(row)]);
    ...
    if (!retain_isa) result.isa.clear();
    return result;
}
...
DivSufsortBuildResult<std::int32_t> build_divsufsort32(
    ...
    return build_divsufsort<std::int32_t>(
        text, sampling_rate, retain_lcp, retain_isa, threads);
}
...
DivSufsortBuildResult<std::int64_t> build_divsufsort64(
    ...
    bool retain_isa,
    std::uint32_t threads) {
    return build_divsufsort<std::int64_t>(
        text, sampling_rate, retain_lcp, retain_isa, threads);
}
